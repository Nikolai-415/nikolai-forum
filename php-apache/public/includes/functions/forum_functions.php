...
function EchoForumsTreeInSelectTag($user_id, $selected_id = "0", $forum_id = 0, $action = "view", $is_topic = 0)
{
    ...
    echo "	<option value=\"0\" ";
    if ($selected_id === "0") echo "selected";
    echo ">";
    echo "Главный форум";
    echo "	</option>";
        ...
        for ($i2 = 0; $i2 < $groups_number; $i2++) {
            ...
            for ($i3 = 0; $i3 < 3; $i3++) {
                ...
                if ($new_i < $permissions_number) {
                    ...
                    if ($user_max_group_rank >= $groups_ranks[$i2] || $cant_delete === 1) {
                        echo "disabled";
                    }
                    echo ">";
                    if ($forum_id === "0" && $action != 'create-forum') {
                        echo "
										<option value=\"0\" ";
                        if ($_POST[$select_name] === null || $_POST[$select_name] === "0" || $cant_delete === 1) echo "selected";
                    } else {
                        echo "
										<option value=\"2\" ";
                        if (($_POST[$select_name] === null || $_POST[$select_name] === "2") && $cant_delete === 0) echo "selected";
                        echo ">
											Не установлено (наследовать)
										</option>
										<option value=\"0\" ";
                        if ($_POST[$select_name] === "0" || $cant_delete === 1) echo "selected";
                    }
                    echo ">
                                                                    Запретить
                                                                </option>";
                    ...
                    echo "<option value=\"1\" ";
                    if ($_POST[$select_name] === "1" && $cant_delete === 0) echo "selected";
                    echo ">
											Разрешить
										</option>
									</select>
        ...
        for ($i2 = 0; $i2 < $groups_number; $i2++) {
            ...
            for ($i3 = 0; $i3 < 3; $i3++) {
                ...
                if ($new_i < $permissions_number2) {
                    ...
                    echo "
										<option value=\"2\" ";
                    if ($_POST[$select_name] === null || $_POST[$select_name] === "2") echo "selected";
                    echo ">
											Не установлено (запретить)
										</option>
										<option value=\"0\" ";
                    if ($_POST[$select_name] === "0") echo "selected";
                    echo ">
											Запретить
										</option>
						";
                    if (($user_permissions[$permissions_fields_names2[$new_i]] == 1) || $disabled == 1) // пользователь не может создать группу с правами, разрешающими то, чего он не может сейчас
                    {
                        echo "
											<option value=\"1\" ";
                        if ($_POST[$select_name] === "1") echo "selected";
                        echo ">
												Разрешить
											</option>
							";
